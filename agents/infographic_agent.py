...
def generate_infographic(state, config):
    thread_id = config.get("configurable", {}).get("thread_id", "default")
    session_data = get_session_data_aggregated(thread_id)
    document_context = session_data.get("extracted_requirements")
    
    if not document_context:
        return {"infographic_generated": False}
    ...
    if msg.images:
        for i, img in enumerate(msg.images):
            ...
            file_path = os.path.join(
                OUTPUT_FOLDER,
                f"infographic_{session_data.get("_id")}.png"
            )
            ...
            state["infographic_url"] = file_path
            # update_infographic_url(session_data.get("_id"), file_path)
